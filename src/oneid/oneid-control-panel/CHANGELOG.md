# oi-control-panel

## 1.4.0

### Minor Changes

- 8e08c43: add admin contorl panel input checks

## 1.3.0

### Minor Changes

- 1ee01b2: add axios request interceptor to inject token from auth context

## 1.2.2

### Patch Changes

- bcae2bc: Bump react-router from 7.10.1 to 7.11.0

## 1.2.1

### Patch Changes

- 7ad49f2: Bump react-router from 7.10.0 to 7.10.1

## 1.2.0

### Minor Changes

- 1c3268e: centralize notification logic and replace local notify state with context
- 3638d75: fix validate notification and add modal pairwise dialog

## 1.1.0

### Minor Changes

- 6d37ff4: Add switch and supporting logic to enable the Pairwise feature
