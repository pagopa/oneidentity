...
import static it.pagopa.oneid.service.utils.ClientUtils.getUpdateMessage;
...
public class ClientRegistrationController {
  ...
  public Response updateClient(
    ...
    //3. Retrieves client from db
    Client client = clientRegistrationService.getClientByClientId(clientId);

    //4. Check if userId in input matches the client userId on db
    ClientUtils.checkUserId(userId, client.getUserId());
    Log.info("client exists for clientId: " + clientId);

    //5. Update client infos
    clientRegistrationService.updateClient(clientRegistrationDTOInput, clientId,
        client.getAttributeIndex(), client.getClientIdIssuedAt(), userId);
    ...
    //6b Add information if redirectUris or metadata-related fields are updated
